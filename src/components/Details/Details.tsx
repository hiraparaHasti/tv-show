  ...
  return (
    <Container style={{ marginTop: "173px", marginLeft: "96px" }}>
      ...
      <CardMedia
        ...
        style={{ width: "269px", marginTop: "27px", marginLeft: "58px" }}
      />
      ...
      >
        <CardContent>
          ...
          >
            <b>Network :</b> {showDetails.network.id},
            <Link to={showDetails.network.name}>
              {showDetails.network.name}
            </Link>
            ,({showDetails.premiered} - {showDetails.ended})
          </Typography>
          <Typography variant="h6" component="div" style={{ fontSize: "18px" }}>
            <b> Schedule :</b> {showDetails.schedule.days} at{" "}
            {showDetails.schedule.time} ({showDetails.runtime}min)
          </Typography>
          <Typography variant="h6" component="div" style={{ fontSize: "18px" }}>
          ...
          <Typography variant="h6" component="div" style={{ fontSize: "18px" }}>
            <b> Official Site : </b>
            <Link to={showDetails.officialSite}>
              {showDetails.officialSite}
            </Link>
          </Typography>
          <Rating
            name="customized-10"
            defaultValue={showDetails.rating.average}
            max={10}
          />
          <Typography style={{ marginLeft: "254px", marginTop: "-31px" }}>
            ( {showDetails.rating.average}K )
          </Typography>
        </CardContent>
      </Card>
    </Container>
